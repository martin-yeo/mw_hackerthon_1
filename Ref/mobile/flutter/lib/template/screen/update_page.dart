...
class _TemplateUpdatePageState extends State<TemplateUpdatePage> {
  ...
  updateTemplateArticle(BuildContext context) async {
    _titleFocus.unfocus();
    _contentsFocus.unfocus();
    num = int.parse(_title!);
    updateResult = await templateService.updateTemplate(context, num, _title!, _content!);
    ...
    if(updateResult != null) {
      ...
      final data = await showBottomModal(context, {
        "title": result ? "게시글 수정 완료" : "게시글 수정 오류",
        "message": result
            ? "게시글 수정이 완료되었습니다."
            : "게시글 수정 시 오류가 발생했습니다. 다시 시도해 주세요.",
        "hasClose": false,
        "hasCancel": false,
        "isDismissible": false,
        "enableDrag": false
                          ...
                          children: <Widget>[
                            Flexible(
                              child: TextButton(
                                ...
                                onPressed: _isSendBtnDisabled! ? () => updateTemplateArticle(context) : null,
